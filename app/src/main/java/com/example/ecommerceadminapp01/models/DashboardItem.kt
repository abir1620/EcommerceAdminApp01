package com.example.ecommerceadminapp01.models

import com.example.ecommerceadminapp01.R

data class DashboardItem(
  val icon: Int,
  val title: String,
  val type: DashboardItemType
)

//for dashboard item's
enum class DashboardItemType{
  ADD_PRODUCT, VIEW_PRODUCT, ORDER, CATEGORY, REPORT, USER, SETTINGS

}

//create a collection that will pass recycler adapter
val dashboardItemList = listOf<DashboardItem>(
  DashboardItem(icon= R.drawable.ic_baseline_add_24, title = "Add Product", type = DashboardItemType.ADD_PRODUCT),
  DashboardItem(icon= R.drawable.ic_baseline_list_24, title = "All Products", type = DashboardItemType.VIEW_PRODUCT),
  DashboardItem(icon= R.drawable.ic_baseline_attach_money_24, title = "Order",type = DashboardItemType.ORDER),
  DashboardItem(icon= R.drawable.ic_baseline_category_24, title = "Category",type = DashboardItemType.CATEGORY),
  DashboardItem(icon= R.drawable.ic_baseline_bar_chart_24, title = "Report",type = DashboardItemType.REPORT),
  DashboardItem(icon= R.drawable.ic_baseline_person_pin_24, title = "Manage Users",type = DashboardItemType.USER),
  DashboardItem(icon= R.drawable.ic_baseline_settings_24, title = "Settings",type = DashboardItemType.SETTINGS)
)

